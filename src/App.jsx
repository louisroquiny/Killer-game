...
function makePlayerCodes(count) {
  const candidates = [];
  for (const word of CHINESE_CODE_WORDS) for (let n = 1; n <= 9; n += 1) candidates.push(`${word}-${n}`);
...
export default function App() {
  ...
  useEffect(() => {
    if (!db || !generalAdminLoggedIn) return undefined;
    return onValue(ref(db, "games"), (snapshot) => setAllGames(snapshot.val() || {}));
  }, [generalAdminLoggedIn]);
  ...
  function gamePublicUrl(id) {
    return `${window.location.origin}${window.location.pathname}?game=${id}`;
  }
  ...
  function loginGeneralAdmin() {
    if (!ensureFirebase()) return;
    if (!GENERAL_ADMIN_PASSWORD) return setMessage("Configure VITE_GENERAL_ADMIN_PASSWORD dans Vercel avant d'utiliser l'admin général.");
    if (generalAdminUser.trim() !== GENERAL_ADMIN_USER || generalAdminPassword !== GENERAL_ADMIN_PASSWORD) return setMessage("Login admin général incorrect.");
    setGeneralAdminLoggedIn(true);
    setMode("generalAdmin");
    setMessage("Admin général connecté.");
  }

  function logoutGeneralAdmin() {
    setGeneralAdminLoggedIn(false);
    setAllGames({});
    setGeneralAdminPassword("");
    setMode("home");
    setMessage("Admin général déconnecté.");
  }

  function openGameAsGeneralAdmin(selectedGame) {
    setGameId(selectedGame.id);
    setGameIdInput(selectedGame.id);
    setAdminCode(selectedGame.adminCode || "");
    setMode("admin");
    window.history.replaceState({}, "", `?game=${selectedGame.id}`);
    setMessage(`Partie ${selectedGame.id} ouverte en mode organisateur.`);
  }

  async function deleteGameById(id) {
    if (!ensureFirebase()) return;
    if (!generalAdminLoggedIn) return setMessage("Connecte-toi comme admin général.");
    await set(ref(db, `games/${id}`), null);
    if (gameId === id) {
      setGame(null);
      setGameId("");
      setGameIdInput("");
    }
    setMessage(`Partie ${id} supprimée.`);
  }
  ...
  async function copyGameLink(id) {
    const ok = await copyText(gamePublicUrl(id));
    setMessage(ok ? `Lien public ${id} copié.` : gamePublicUrl(id));
  }
